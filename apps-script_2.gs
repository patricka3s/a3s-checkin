...
const PROFILE_COLUMNS = [
  ...
  // Photo consent (private — organizer-only, checked once at first check-in
  // on a device; see PhotoConsent handling in doPost/upsertProfile). Kept
  // as the LAST column so appending it here doesn't shift any existing
  // sheet's columns — see getOrCreateSheet's header-backfill below.
  'PhotoConsent'
];
...
function upsertProfile(ss, body) {
  ...
  const row = PROFILE_COLUMNS.map(function (col, idx) {
    switch (col) {
      ...
      // The client sends this as body.photoConsent (camelCase, like guid/
      // displayName above) rather than body.PhotoConsent, so it needs its
      // own case — the generic default below matches on the column name
      // verbatim and would otherwise never see it.
      case 'PhotoConsent':
        return (body.photoConsent && body.photoConsent !== '') ? body.photoConsent : (existingRow ? existingRow[idx] : '');
      default:
...
function getOrCreateSheet(ss, name, columns) {
  ...
  // Backfill: if `columns` has grown since this sheet was first created
  // (e.g. PhotoConsent added to PROFILE_COLUMNS after Profiles already
  // existed on a live deployment), extend the header row to match rather
  // than silently writing values into unlabeled trailing columns. Only
  // handles columns *appended* to the end — never reorder/insert an
  // existing column, since this matches by position, not by name.
  const existingWidth = sheet.getLastColumn();
  if (existingWidth < columns.length) {
    const newHeaders = columns.slice(existingWidth);
    sheet.getRange(1, existingWidth + 1, 1, newHeaders.length).setValues([newHeaders]);
  }
  return sheet;
}
